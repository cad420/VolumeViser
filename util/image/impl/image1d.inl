...
VUTIL_BEGIN
    ...
    {
        return image1d_view_t<T,false>(data.get_subview(tvec<int,1>{beg_x},tvec<int,1>{end_x - beg_x}));
    }

    template <typename T>
    image1d_view_t<T, true> image1d_t<T>::sub_view( int beg_x, int end_x ) const
    {
        return image1d_view_t<T,true>(data.get_subview(tvec<int,1>{beg_x},tvec<int,1>{end_x - beg_x}));
    }

    template <typename T>
    image1d_view_t<T, true> image1d_t<T>::sub_view_const( int beg_x, int end_x ) const
    {
        return image1d_view_t<T,true>(data.get_const_subview(tvec<int,1>{beg_x},tvec<int,1>{end_x - beg_x}));
    }
