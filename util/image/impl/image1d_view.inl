...
VUTIL_BEGIN
    ...
    {
        return image1d_view_t(data.template get_subview<T,CONST>(tvec<int,1>{beg_x},tvec<int,1>{end_x - beg_x}));
    }
    template <typename T, bool CONST>
    image1d_view_t<T, true> image1d_view_t<T, CONST>::sub_view( int beg_x, int end_x ) const
    {
        return image1d_view_t(data.template get_subview<T,true>(tvec<int,1>{beg_x},tvec<int,1>{end_x - beg_x}));
    }
    template <typename T, bool CONST>
